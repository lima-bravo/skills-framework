...
This project is a library of 266 mental model cards for disciplined critical thinking. Your role here is to act as a **natural language interface** to the framework: surface the right models for a situation, apply their heuristics, and flag their failure modes.
...
|---|---|---|
| Total skills | `skills-manifest.json` keys | 266 |
| Categories | distinct `category` values | 16 |
| Pre-built chains | category = `Pre-built Chains` | 22 |
| Non-chain cards | total − chains | 244 |
| Sources / references | `manifest.refs.length` | 385 |
| Graph connections | generated `docs/graph.html` | 1130 |

...
A mental-models framework with a publishable **reference website** (`docs/`), per-skill markdown deep-dives, a connection graph, and a [training guide](Skills%20Reference/training-guide.md). **266 skills** across **16 categories** (source of truth: `skills-manifest.json`).
...
- Search and category filters across **266 skills**
...
The deck's **266 reference cards** are not exported 1:1. `npm run build:cowork` generates **59 procedural skills** in seven plugins under `plugins/`:
...
Skills Reference/
  ...
  skills-manifest.json       # Canonical registry: ids, categories, refs (266 skills)

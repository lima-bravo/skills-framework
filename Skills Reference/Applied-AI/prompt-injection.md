...
**Read/write agent separation.** See [Minimal Capability Principle](../Applied-AI/minimal-capability-principle.md) for the full design discipline — in short, the agent that reads untrusted external content is architecturally separated from the agent that writes outputs or takes actions, with no shared tool grant that would let a successfully injected reader act.

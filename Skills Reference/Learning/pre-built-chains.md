---
id: 105
name: Pre-built Chains
category: Learning
cardType: standard
tagline: Structured multi-step reasoning protocols that sequence mental models in optimal order for recurring high-stakes decision types.
connections:
  - id: 86
    rationale: model chaining is the general practice of sequencing multiple mental models; pre-built chains are the specific, curated form of that practice applied to known decision types.
  - id: 0
    rationale: 'pre-built chains encode first principles reasoning about each decision type: they are the distilled answer to "what are the irreducible questions to ask about a pricing / hiring / market entry decision?"'
  - id: 1
    rationale: every pre-built chain includes an inversion step (pre-mortem or assumption check) as a structural disconfirmation mechanism.
  - id: 55
    rationale: the primary purpose of chain structure is to prevent confirmation bias from narrowing the analysis to only the models that support the preferred conclusion.
  - id: 16
    rationale: "chains are most valuable for irreversible or high-stakes decisions (two-way doors logic: reversible decisions deserve fast analysis; irreversible ones deserve chain-level rigor)."
references:
  - title: "Poor Charlie's Almanack: The Wit and Wisdom of Charles T. Munger"
    authorYear: Charlie Munger, ed. Peter Kaufman (2005)
    supports: Munger's checklist-based approach to decision-making is the conceptual ancestor of pre-built chains.
  - title: "How to Solve It: A New Aspect of Mathematical Method"
    authorYear: George Pólya (1945)
    supports: "the original structured thinking sequence: a pre-built chain for mathematical problem-solving."
---
...
*This card explains the concept of a pre-built chain. To use the actual chains — Pricing Decision, Market Entry, Hiring Decision, and nineteen others — open the "Pre-built Chains" category in the card deck.*
...
A founder is deciding whether to raise a Series A or continue bootstrapping. A colleague facing the same choice last year spent an evening deciding which models even applied — she skips that step entirely and opens the Investment Decision chain off the shelf. The protocol was built once, by someone who had already made (and tested) the sequencing decision: diagnose the real goal before touching incentives, check the incentives before checking the runway math, and never skip the pre-mortem. She isn't discovering that Incentives matters here — the chain's author already decided it always matters for this decision type, in this position, and simply hands her the slot to fill in. Step 1 (First Principles) reveals her actual goal: not just fund the business, but reach a specific revenue milestone within 18 months. Step 3 (Incentives) exposes a tension: venture capital will align investor and founder incentives around hypergrowth, but her business model works better at moderate, profitable scale. Step 5 (Margin of Safety) shows that current runway is 14 months — not 18, as she had assumed. Step 7 (Pre-mortem) generates the most useful output: the most likely failure scenario is not that the raise fails, but that it succeeds, changes the incentive structure, and forces a growth strategy that destroys the business model's natural margins. She decides to pursue a strategic partnership instead of a VC raise — reached in one sitting, using a protocol she didn't have to design, only fill in honestly.

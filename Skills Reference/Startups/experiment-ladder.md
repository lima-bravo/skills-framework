...
**Product** (medium cost, stronger evidence). Testing whether a real — if minimal — product solves the problem satisfactorily. Methods: wizard-of-oz, concierge, single-feature prototype, MVP. What it can answer: "Does this product solve the problem well enough that customers use it and return?" What it cannot answer: whether the business model around the product is economically viable at scale.

The two manual-delivery methods test different things and should be chosen deliberately. *Wizard of Oz*: the human operator is hidden — the customer believes they are using an automated system. Use this when the hypothesis includes the *experience* of automation (speed, consistency, the feel of the interaction). *Concierge*: the human is visible — the customer knows a person is performing the work. Use this when you only need to know whether the outcome is valued, independent of how it is delivered. A Wizard of Oz result tells you the automated experience works; a Concierge result tells you the outcome is worth having. Both are valid product-rung experiments, but conflating them produces evidence for the wrong question.

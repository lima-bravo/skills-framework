...
Two variants are worth distinguishing. *Wizard of Oz*: the human is hidden — the customer believes they are interacting with an automated system. *Concierge*: the human is visible — the customer knows a person is performing the work. They test different things. Wizard of Oz tests whether the *automated experience* works: latency, interaction model, the feeling of the system. Concierge tests whether the *outcome* is valued at all, independent of delivery mechanism. Use Wizard of Oz when the believability of automation is part of the hypothesis. Use Concierge when you only need to know whether the outcome is worth having.

The specific failure mode of Mechanical Turk is the **transition trap**: the team discovers that hiring humans to deliver the experience is cheaper and more reliable than building the underlying technology, so the "pretotype" becomes the product. This is not always wrong — some businesses are deliberately human-powered. But if the hypothesis was that the product would be automated, running a manual operation indefinitely is not validation; it is evasion. Set an explicit date or volume threshold at which the pretotype must convert to a built system or be killed.
...
- **The transition trap (Mechanical Turk specific).** The Wizard of Oz succeeds at generating demand — customers want the product. But the human-powered version also works well enough to retain them. The team never builds the underlying automation, because the pretotype is already generating revenue. If the original hypothesis included automated delivery (for scale, margin, or capability reasons), a sustainable manual operation is not validation — it is a different business. Set a conversion deadline before running the pretotype: at what volume or date must this convert to a built system, or stop?
